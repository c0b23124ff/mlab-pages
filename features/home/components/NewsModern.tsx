...
interface NewsModernProps {
  onNavigate: (page: string) => void;
  onSelectNews: (id: string) => void;
}

export const NewsModern = ({ onNavigate, onSelectNews }: NewsModernProps) => {
      ...
      <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          <div>
            ...
            <p className="text-xl text-[#344F1F]/80">News & Topics</p>
          </div>
          <button
          ...
          {newsItems.map((item, index) => {
            ...
            return (
              <motion.div
                ...
                className="group cursor-pointer focus-within:ring-2 focus-within:ring-[#F4991A] focus-within:ring-offset-2 rounded-2xl"
                tabIndex={0}
                onKeyDown={(e) => e.key === 'Enter' && onSelectNews(item.id)}
              >
                <div className="relative h-full bg-white rounded-2xl overflow-hidden shadow-md border border-[#344F1F]/10 hover:shadow-xl hover:border-[#F4991A]/40 transition-all duration-300">
                  {/* Image */}
                  ...
                  <div className="p-6">
                    ...
                    {item.summary && (
                      <p className="text-sm text-[#344F1F]/70 mb-4 line-clamp-2">{item.summary}</p>
                    )}
        ...
        >
          <button
            onClick={() => onNavigate('news')}
            className="inline-flex items-center gap-2 bg-[#F4991A] hover:bg-[#D88615] text-white px-8 py-4 rounded-full font-bold transition-colors"
          >
            View All News
